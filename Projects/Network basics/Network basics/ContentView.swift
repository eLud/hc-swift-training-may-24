...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .task {
            loadData()
            await loadDataSwiftAsync()
        }
    }


    /// Former syntax, using closure as callback
    func loadData() {
    ...
    /// Syntax using the new Swift async feature
    func loadDataSwiftAsync() async {
        guard let url = URL(string: "https://itunes.apple.com/search?term=pink+floyd&entity=song") else { return }
        let request = URLRequest(url: url)

        do {
            let (data, _) = try await URLSession.shared.data(for: request)
            let decodedResponse = try JSONDecoder().decode(ITMSResponse.self, from: data)
            self.results = decodedResponse.results
        } catch {
            print(error)
        }
